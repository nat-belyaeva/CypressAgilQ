class LogInPage {
    ...
    verifyUserCannotSignInWithIncorrectData() {
        this.elements.signInForm().should('be.visible');
        // Поле signIn будет видно, если ты не авторизовался (поля password и logIn останутся)
    };

    verifyUserCanSignInWithCorrectData() {
        this.elements.signInForm.should('not.be.visible');
        // Если авторизовался успешно, то этих полей будет не видно.
    }
    ...
    clickLogInBtn() {
        this.elements.getLogInBtn().should('be.visible').click({ force: true }).click({ force: true });
    };
    ...
    clickOkBtnConfirmPopUp() {
        this.elements.getOkBtnConfirmPopUp().should('be.visible').click({ force: true });
    };
